  ...
  <div class="min-h-screen bg-gray-50 text-gray-900 font-sans py-10 px-4">
    <div class="max-w-5xl mx-auto">
      ...
      <nav class="text-xs text-gray-400 mb-4 font-medium">
        ...
        <span class="text-gray-600 font-semibold">상점 지도</span>
      </nav>
      ...
      <div class="bg-white p-6 rounded-2xl border border-gray-200/70 shadow-sm mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <span>🛍️</span> 서울 상점 지도
          </h1>
          <p class="text-xs text-gray-400 mt-1">
            지도 위 핀을 클릭해 상점 정보와 네이버 검색 연결 링크를 확인해 보세요.
          </p>
        </div>
...
watch(selectedShop, async (newShop) => {
  if (newShop) {
    try {
      const keyword = newShop.title.trim()
      relatedPosts.value = await postApi.searchPosts(keyword)
    } catch (error) {
      console.error('관련 게시글 호출 실패:', error)
      relatedPosts.value = []
    }
  } else {
    relatedPosts.value = []
...
onMounted(async () => {
  try {
    mockShops.value = await placeApi.getPlaces('shopping')
    loadNaverMapScript()
  } catch (error) {
    console.error('쇼핑 명소 데이터 호출 실패:', error)
